      ...
      .then(devices => (
        Promise.all(map(devices, device => (
          db.query(queries.select_device_info, [device.id, device.id])
            .then(result => {
              // The resulting device might not have recorded any hikes yet, meaning there is no carrier,
              // manufacturer, or os information for that device. The data is organized this way so that
              // if someone changes their device in the future, we can track that change through their hikes
